...
def main():
    ...
    with st.sidebar:
        ...
        # 更新选择的股票代码
        if stock_code and stock_code != st.session_state.selected_stock:
            # 简单验证：6位数字
    ...
    # 验证股票代码
    if not selected_stock or not (selected_stock.isdigit() and len(selected_stock) == 6):
        st.warning("请输入有效的6位数字股票代码")
        st.stop()
    
    # 获取股票名称
    with st.spinner('正在获取股票信息...'):
        stock_name = get_stock_name(selected_stock)
